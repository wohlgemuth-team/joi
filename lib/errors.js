...
exports.Err = class {
    ...
    toString() {
        ...
        let wrapArrays = Utils.reach(localized, 'messages.wrapArrays');
        ...
        if (!hasKey && !skipKey) {
            const localizedKey = Utils.reach(localized, 'key');
        ...
        return format.replace(/\{\{(\!?)([^}]+)\}\}/g, ($0, isSecure, name) => {
            ...
            const value = Utils.reach(this.context, name);
            const normalized = internals.stringify(value, wrapArrays);
            return (isSecure && this.options.escapeHtml ? Utils.escapeHtml(normalized) : normalized);
        });
    }
...
internals.annotate = function (stripColorCodes) {
    ...
    const obj = Utils.clone(this._object || {});
