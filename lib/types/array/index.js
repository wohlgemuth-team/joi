...
internals.Array = class extends Any {
    ...
    items(...schemas) {
        ...
        Utils.flatten(schemas).forEach((type, index) => {
    ...
    unique(comparator) {
        ...
        Utils.assert(comparator === undefined ||
        ...
        return this._test('unique', settings, function (value, state, options) {
            ...
            const compare = settings.comparator || Utils.deepEqual;

            for (let i = 0; i < value.length; ++i) {
                const item = settings.path ? Utils.reach(value[i], settings.path) : value[i];
    ...
    _fillOrderedErrors(errors, ordereds, state, options) {
        ...
        for (let i = 0; i < ordereds.length; ++i) {
            const presence = Utils.reach(ordereds[i], '_flags.presence');
